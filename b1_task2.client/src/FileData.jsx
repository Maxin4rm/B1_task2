...
// Компонент для отображения баланса одного аккаунта
...
const ClassInfo = ({ classData }) => (
    <tbody>
        <tr>
        <th align='center' colSpan={8}>{classData.class.className}</th> {/* Укажите количество столбцов, на которые должна растягиваться ячейка */}
    </tr>
    ...
    return (
        <div>
            ...
            {fileData?.map((item, index) => (
                <div key={index}>
                    <h2>{item.account.class.className}</h2>
                    <table>
                        <thead>
                            <tr>
                                <th>Account ID</th>
                                <th>Account Number</th>
                                <th>Incoming Active</th>
                                <th>Incoming Passive</th>
                                <th>Debit</th>
                                <th>Credit</th>
                                <th>Outgoing Active</th>
                                <th>Outgoing Passive</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr>
                                ...
                                <td>{item.incomingActive}</td>
                                <td>{item.incomingPassive}</td>
                                <td>{item.debit}</td>
                                <td>{item.credit}</td>
                                <td>{item.outgoingActive}</td>
                                <td>{item.outgoingPassive}</td>
                            </tr>
                        </tbody>
                    </table>
                    <div>
                        <strong>Баланс класса:</strong> 
                        Incoming Active: {item.account.class.classBalance.incomingActive}, 
                        Outgoing Active: {item.account.class.classBalance.outgoingActive}
                    </div>
                </div>
            ))}
        </div>
